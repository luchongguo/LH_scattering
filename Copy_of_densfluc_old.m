function [snperm,ds,cnz,cnr,cm,xrandom]=densfluc_old(z,r,phi,cnz,cnr,cm,tstep)
...
if mode==1
    ...
    deltt=tstep;
    [~,ddt]=calc_ddd1_old(z,r,phi,cnz,cnr,cm);     %dddz,dddr,dddphi等,此时还不是散射后的值了
...
if mode==2 & imag(cnperpf)~=0  
    ...
    %deltt=tavg/sumss;
    deltt=tstep;
    [~,ddt]=calc_ddd1_old(z,r,phi,cnz,cnr,cm);      %dddz,dddr,dddphi等,此时已经是散射后的值了
